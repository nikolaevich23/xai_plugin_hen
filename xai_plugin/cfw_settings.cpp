...
int toggle_coldboot()
{
	...
	// Check if colboot.raf.mod exists and colboot.raf.ori, del mod
	if ((cellFsStat(coldboot_modded, &statinfo) == CELL_OK) && (cellFsStat(coldboot_original, &statinfo) == CELL_OK))
	{
		cellFsUnlink(coldboot_modded);
		cellFsUnlink(coldboot_modded_multi_ac3);
		cellFsUnlink(coldboot_modded_stereo_ac3);
	}

	// Check if colboot.raf.mod exists and swap it
	if (cellFsStat(coldboot_modded, &statinfo) == CELL_OK)
	{
		ret = cellFsRename(coldboot_file, coldboot_original);
		ret_m = cellFsRename(coldboot_file_multi_ac3, coldboot_original_multi_ac3);
		ret_s = cellFsRename(coldboot_file_stereo_ac3, coldboot_original_stereo_ac3);
		if ((ret != CELL_OK) && (ret_m != CELL_OK) && (ret_s != CELL_OK))
			showMessage("msg_raf_swap_ori_error", (char*)XAI_PLUGIN, (char*)TEX_ERROR);

		ret = cellFsRename(coldboot_modded, coldboot_file);
		ret_m = cellFsRename(coldboot_modded_multi_ac3, coldboot_file_multi_ac3);
		ret_s = cellFsRename(coldboot_modded_stereo_ac3, coldboot_file_stereo_ac3);
		if ((ret != CELL_OK) && (ret_m != CELL_OK) && (ret_s != CELL_OK))
	...
	else if (cellFsStat(coldboot_original, &statinfo) == CELL_OK)
	{
		ret = cellFsRename(coldboot_file, coldboot_modded);
		ret_m = cellFsRename(coldboot_file_multi_ac3, coldboot_modded_multi_ac3);
		ret_s = cellFsRename(coldboot_file_stereo_ac3, coldboot_modded_stereo_ac3);
		if ((ret != CELL_OK) && (ret_m != CELL_OK) && (ret_s != CELL_OK))
			showMessage("msg_raf_swap_mod_error", (char*)XAI_PLUGIN, (char*)TEX_ERROR);

		ret = cellFsRename(coldboot_original, coldboot_file);
		ret_m = cellFsRename(coldboot_original_multi_ac3, coldboot_file_multi_ac3);
		ret_s = cellFsRename(coldboot_original_stereo_ac3, coldboot_file_stereo_ac3);
		if ((ret != CELL_OK) && (ret_m != CELL_OK) && (ret_s != CELL_OK))
			showMessage("msg_raf_swap_ori_error", (char*)XAI_PLUGIN, (char*)TEX_ERROR);
...
int status_ftp()
{
	...
	char ip[0x10] = { NULL };
	char name[120], filename[120], ftpname[120];
	CellFsStat stat;
	...
	for (slot = 1; slot < MAX_BOOT_PLUGINS; slot++)
	{
		...
		//sprintf_(ftpname, "%d %s", slot, (int)name);
		//log("slot %s\n", ftpname);

		if (strlen(name) == 0 && load_slot == 0)
			load_slot = slot;

		if (!strcmp(name, FTPD) || !strcmp(name, WWWD))
		{
			memset(ip, 0, sizeof(ip));
			sceNetCtlGetInfoVsh(0x10, ip);
			sprintf_(ftpname, "%s\n%s", (ip[0] == NULL ? (int)"-" : (int)ip), (int)filename);
			int string = RetrieveString("msg_ftp_sprx_enabled", (char*)XAI_PLUGIN);
			swprintf_(wchar_string, 120, (wchar_t*)string, (int)ftpname);
			PrintString(wchar_string, (char*)XAI_PLUGIN, (char*)TEX_INFO2);
			return 1;
		}
	}

	showMessage("msg_ftp_sprx_disabled", (char *)XAI_PLUGIN, (char *)TEX_INFO2);
	return 1;
}
...
int toggle_update()
{
	...
	// Check if .mod exists and .ori, del mod
	if ((cellFsStat(upd_modded, &statinfo) == CELL_OK) && (cellFsStat(upd_original, &statinfo) == CELL_OK))
	{
		cellFsUnlink(upd_modded);
	}

	// Check if ori exists and swap it
	if (cellFsStat(upd_original, &statinfo) == CELL_OK)
	{
		ret = cellFsRename(upd_file, upd_modded);
		if (ret != CELL_OK)
			showMessage("msg_upd_swap_mod_error", (char*)XAI_PLUGIN, (char*)TEX_ERROR);

		ret = cellFsRename(upd_original, upd_file);
		if (ret != CELL_OK)
	...
	else if (cellFsStat(upd_modded, &statinfo) == CELL_OK)
	{
	...
	{
		showMessage("msg_no_upd_detected", (char*)XAI_PLUGIN, (char*)TEX_ERROR);
		return 1;
	}
...
void toggle_generic(char* path_to_file, char* path_icon_to, char* msg)
{
	int ret = 0;
	int fd = 0;
	char enable[30];
	...
	char on[4] = ".on";
	char off[5] = ".off";
	char path_to_file_on[40];
	char path_to_file_off[40];
	sprintf_(path_to_file_on, "%s%s", (int)path_to_file, (int)on);
	sprintf_(path_to_file_off, "%s%s", (int)path_to_file, (int)off);

	CellFsStat stat;
	ret = cellFsStat(path_to_file_off, &stat);
	if (ret != CELL_OK)
	{
		cellFsUnlink(path_to_file_on);
		cellFsOpen(path_to_file_off, CELL_FS_O_CREAT | CELL_FS_O_RDWR, &fd, 0, 0);
		cellFsClose(fd);
		filecopy("/dev_hdd0/hen/off.png", path_icon_to);
		sprintf_(disable, "%s%s", (int)msg, (int)dis);
		showMessage(disable, (char*)XAI_PLUGIN, (char *)TEX_INFO);
	}
	else
	{
		cellFsUnlink(path_to_file_off);
		cellFsOpen(path_to_file_on, CELL_FS_O_CREAT | CELL_FS_O_RDWR, &fd, 0, 0);
		cellFsClose(fd);
		filecopy("/dev_hdd0/hen/on.png", path_icon_to);
		sprintf_(enable, "%s%s", (int)msg, (int)enb);
		showMessage(enable, (char*)XAI_PLUGIN, (char *)TEX_INFO2);
	}
	reload_xmb_n();
}
...
void toggle_hen_xmb()
{
	toggle_generic("/dev_hdd0/hen/hen_xmb", "/dev_hdd0/hen/hen_xmb.png", "HEN_in_XMB");
}
...
{
	toggle_generic("/dev_hdd0/hen/clear_web_history", "/dev_hdd0/hen/clear_web_history.png", "Clear_web_history");
}

void toggle_hen_clear_web_auth_cache()
{
	toggle_generic("/dev_hdd0/hen/clear_web_auth_cache", "/dev_hdd0/hen/clear_web_auth_cache.png", "Clear_auth_cache");
}

void toggle_hen_clear_web_cookie()
{
	toggle_generic("/dev_hdd0/hen/clear_web_cookie", "/dev_hdd0/hen/clear_web_cookie.png", "Clear_cookie");
}

void toggle_hen_clear_info()
{
	toggle_generic("/dev_hdd0/hen/clear_info", "/dev_hdd0/hen/clear_info.png", "Clear_info");
}

void toggle_hen_hotkeys()
{
	toggle_generic("/dev_hdd0/hen/hotkeys", "/dev_hdd0/hen/hotkeys.png", "Hotkeys");
}

void toggle_hen_debug_set()
{
	toggle_generic("/dev_hdd0/hen/debug_menu", "/dev_hdd0/hen/dm.png", "msg_toggle_sysconf");
}

void toggle_hen_trophy()
{
	toggle_generic("/dev_hdd0/hen/trophy", "/dev_hdd0/hen/trophy.png", "trophy");
}

void toggle_hen_gameboot()
{
	toggle_generic("/dev_hdd0/hen/gameboot", "/dev_hdd0/hen/gameboot.png", "gameboot");
}

void toggle_hen_reload()
{
	toggle_generic("/dev_hdd0/hen/hen_reload", "/dev_hdd0/hen/hen_reload.png", "hen_reload");
}

void toggle_hen_ofw()
{
	toggle_generic("/dev_hdd0/hen/hen_ofw", "/dev_hdd0/hen/hen_ofw.png", "Utlites");
	int rt = 0;
	int ff = 0;
	CellFsStat stat;
	...
	if (rt != CELL_OK)
	{
		...
		cellFsClose(ff);
		filecopy("/dev_flash/hen/xml/ofw_m.xml", "/dev_hdd0/hen/ofw_m.xml");
	}
	else
	{
		cellFsUnlink("/dev_hdd0/hen/ofw_m.xml");
		filecopy("/dev_flash/hen/xml/empty.xml", "/dev_hdd0/hen/ofw_m.xml");
	}
	reload_xmb_n();
}
...
void toggle_hen_mag()
{
	toggle_generic("/dev_hdd0/hen/hen_mag", "/dev_hdd0/hen/hen_mag.png", "shops");
	int rt = 0;
	int ff = 0;
	CellFsStat stat;
	...
	if (rt != CELL_OK)
	{
		...
		cellFsClose(ff);
		filecopy("/dev_flash/hen/xml/mag_on.xml", "/dev_hdd0/hen/mag.xml");
	}
	else
	{
		//cellFsUnlink("/dev_hdd0/hen/mag.xml");
		filecopy("/dev_flash/hen/xml/empty.xml", "/dev_hdd0/hen/mag.xml");
	}
	reload_xmb_n();
}

void toggle_hen_folder_xmb()
{
	toggle_generic("/dev_hdd0/hen/hen_folder_xmb", "/dev_hdd0/hen/hen_folder_xmb.png", "HEN_in_XMB");
	int rt = 0;
	int ff = 0;
	CellFsStat stat;
	rt = cellFsStat("/dev_hdd0/hen/hen_folder_xmb.off", &stat);
	if (rt != CELL_OK)
	{
		cellFsOpen("/dev_hdd0/hen/mag.xml", CELL_FS_O_CREAT | CELL_FS_O_RDWR, &ff, 0, 0);
		cellFsClose(ff);
		filecopy("/dev_flash/hen/xml/mode.xml", "/dev_hdd0/hen/mode.xml");
	}
	else
	{
		//cellFsUnlink("/dev_hdd0/hen/mode.xml");
		filecopy("/dev_flash/hen/xml/empty.xml", "/dev_hdd0/hen/mode.xml");
	}
	reload_xmb_n();
}
...
{
	toggle_generic("/dev_hdd0/hen/hen_gps3", "/dev_hdd0/hen/hen_gps3.png", "gps3");
	int rt = 0;
	int ff = 0;
	CellFsStat stat;
	rt = cellFsStat("/dev_hdd0/hen/hen_gps3.off", &stat);
	if (rt != CELL_OK)
	{
		filecopy("/dev_flash/hen/xml/gall-on.xml", "/dev_rewrite/vsh/resource/explore/xmb/category_game.xml");
		filecopy("/dev_flash/hen/xml/gps3.xml", "/dev_hdd0/hen/gps3.xml");
	}
	else
	{
		cellFsUnlink("/dev_hdd0/hen/gps3.xml");
		//filecopy("/dev_flash/hen/xml/empty.xml", "/dev_hdd0/hen/gps3.xml");
	}
	reload_xmb_n();
}

void toggle_hen_gps2()
{
	toggle_generic("/dev_hdd0/hen/hen_gps2", "/dev_hdd0/hen/hen_gps2.png", "gps2");
	int rt = 0;
	int ff = 0;
	CellFsStat stat;
	rt = cellFsStat("/dev_hdd0/hen/hen_gps2.off", &stat);
	if (rt != CELL_OK)
	{
		filecopy("/dev_flash/hen/xml/gall-on.xml", "/dev_rewrite/vsh/resource/explore/xmb/category_game.xml");
		filecopy("/dev_flash/hen/xml/gps2.xml", "/dev_hdd0/hen/gps2.xml");
	}
	else
	{
		cellFsUnlink("/dev_hdd0/hen/gps2.xml");
		//filecopy("/dev_flash/hen/xml/empty.xml", "/dev_hdd0/hen/gps2.xml");
	}
	reload_xmb_n();
}

void toggle_hen_gps1()
{
	toggle_generic("/dev_hdd0/hen/hen_gps1", "/dev_hdd0/hen/hen_gps1.png", "gps1");
	int rt = 0;
	int ff = 0;
	CellFsStat stat;
	rt = cellFsStat("/dev_hdd0/hen/hen_gps1.off", &stat);
	if (rt != CELL_OK)
	{
		filecopy("/dev_flash/hen/xml/gall-on.xml", "/dev_rewrite/vsh/resource/explore/xmb/category_game.xml");
		filecopy("/dev_flash/hen/xml/gps1.xml", "/dev_hdd0/hen/gps1.xml");
	}
	else
	{
		cellFsUnlink("/dev_hdd0/hen/gps1.xml");
		//filecopy("/dev_flash/hen/xml/empty.xml", "/dev_hdd0/hen/gps1.xml");
	}
	reload_xmb_n();
}

void toggle_hen_gpsp()
{
	toggle_generic("/dev_hdd0/hen/hen_gpsp", "/dev_hdd0/hen/hen_gpsp.png", "gpsp");
	int rt = 0;
	int ff = 0;
	CellFsStat stat;
	rt = cellFsStat("/dev_hdd0/hen/hen_gpsp.off", &stat);
	if (rt != CELL_OK)
	{
		filecopy("/dev_flash/hen/xml/gall-on.xml", "/dev_rewrite/vsh/resource/explore/xmb/category_game.xml");
		filecopy("/dev_flash/hen/xml/gpsp.xml", "/dev_hdd0/hen/gpsp.xml");
	}
	else
	{
		cellFsUnlink("/dev_hdd0/hen/gpsp.xml");
		//filecopy("/dev_flash/hen/xml/empty.xml", "/dev_hdd0/hen/gpsp.xml");
	}
	reload_xmb_n();
}

void toggle_hen_gpsn()
{
	toggle_generic("/dev_hdd0/hen/hen_gpsn", "/dev_hdd0/hen/hen_gpsn.png", "gpsn");
	int rt = 0;
	int ff = 0;
	CellFsStat stat;
	rt = cellFsStat("/dev_hdd0/hen/hen_gpsn.off", &stat);
	if (rt != CELL_OK)
	{
		filecopy("/dev_flash/hen/xml/gall-on.xml", "/dev_rewrite/vsh/resource/explore/xmb/category_game.xml");
		filecopy("/dev_flash/hen/xml/gpsn.xml", "/dev_hdd0/hen/gpsn.xml");
	}
	else
	{
		cellFsUnlink("/dev_hdd0/hen/gpsn.xml");
		//filecopy("/dev_flash/hen/xml/empty.xml", "/dev_hdd0/hen/gpsn.xml");
	}
	reload_xmb_n();
}
...
void toggle_hen_ghb()
{
	toggle_generic("/dev_hdd0/hen/hen_ghb", "/dev_hdd0/hen/hen_ghb.png", "ghb");
	int rt = 0;
	int ff = 0;
	CellFsStat stat;
	rt = cellFsStat("/dev_hdd0/hen/hen_ghb.off", &stat);
	if (rt != CELL_OK)
	{
		filecopy("/dev_flash/hen/xml/gall-on.xml", "/dev_rewrite/vsh/resource/explore/xmb/category_game.xml");
		filecopy("/dev_flash/hen/xml/ghb.xml", "/dev_hdd0/hen/ghb.xml");
	}
	else
	{
		cellFsUnlink("/dev_hdd0/hen/ghb.xml");
		//filecopy("/dev_flash/hen/xml/empty.xml", "/dev_hdd0/hen/ghb.xml");
	}
	reload_xmb_n();
}
...
void toggle_hen_grg()
{
	toggle_generic("/dev_hdd0/hen/hen_grg", "/dev_hdd0/hen/hen_grg.png", "grg");
	int rt = 0;
	int ff = 0;
	CellFsStat stat;
	rt = cellFsStat("/dev_hdd0/hen/hen_grg.off", &stat);
	if (rt != CELL_OK)
	{
		filecopy("/dev_flash/hen/xml/gall-on.xml", "/dev_rewrite/vsh/resource/explore/xmb/category_game.xml");
		filecopy("/dev_flash/hen/xml/grg.xml", "/dev_hdd0/hen/grg.xml");
	}
	else
	{
		cellFsUnlink("/dev_hdd0/hen/grg.xml");
		//filecopy("/dev_flash/hen/xml/empty.xml", "/dev_hdd0/hen/grg.xml");
	}
	reload_xmb_n();
}

void toggle_hen_gall()
{
	toggle_generic("/dev_hdd0/hen/hen_gall", "/dev_hdd0/hen/hen_gall.png", "gall");
	int rt = 0;
	int ff = 0;
	CellFsStat stat;
	rt = cellFsStat("/dev_hdd0/hen/hen_gall.off", &stat);
	if (rt != CELL_OK)
	{
		/*cellFsOpen("/dev_rewrite/vsh/resource/explore/xmb/category_game.xml", CELL_FS_O_CREAT | CELL_FS_O_RDWR, &ff, 0, 0);
		cellFsClose(ff);*/
		filecopy("/dev_flash/hen/xml/gall-on.xml", "/dev_rewrite/vsh/resource/explore/xmb/category_game.xml");
		cellFsUnlink("/dev_hdd0/hen/hen_gps3.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_gps3.on");
		cellFsUnlink("/dev_hdd0/hen/hen_gps2.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_gps2.on");
		cellFsUnlink("/dev_hdd0/hen/hen_gps1.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_gps1.on");
		cellFsUnlink("/dev_hdd0/hen/hen_gpsp.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_gpsp.on");
		cellFsUnlink("/dev_hdd0/hen/hen_gpsn.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_gpsn.on");
		cellFsUnlink("/dev_hdd0/hen/hen_ghb.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_ghb.on");
		cellFsUnlink("/dev_hdd0/hen/hen_grg.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_grg.on");
		filecopy("/dev_hdd0/hen/on.png", "/dev_hdd0/hen/hen_gps3.png");
		filecopy("/dev_hdd0/hen/on.png", "/dev_hdd0/hen/hen_gps2.png");
		filecopy("/dev_hdd0/hen/on.png", "/dev_hdd0/hen/hen_gps1.png");
		filecopy("/dev_hdd0/hen/on.png", "/dev_hdd0/hen/hen_gpsp.png");
		filecopy("/dev_hdd0/hen/on.png", "/dev_hdd0/hen/hen_gpsn.png");
		filecopy("/dev_hdd0/hen/on.png", "/dev_hdd0/hen/hen_ghb.png");
		filecopy("/dev_hdd0/hen/on.png", "/dev_hdd0/hen/hen_grg.png");
		filecopy("/dev_flash/hen/xml/gps3.xml", "/dev_hdd0/hen/gps3.xml");
		filecopy("/dev_flash/hen/xml/gps2.xml", "/dev_hdd0/hen/gps2.xml");
		filecopy("/dev_flash/hen/xml/gps1.xml", "/dev_hdd0/hen/gps1.xml");
		filecopy("/dev_flash/hen/xml/gpsp.xml", "/dev_hdd0/hen/gpsp.xml");
	...
	else
	{
		/*cellFsOpen("/dev_rewrite/vsh/resource/explore/xmb/category_game.xml", CELL_FS_O_CREAT | CELL_FS_O_RDWR, &ff, 0, 0);
		cellFsClose(ff);*/
		filecopy("/dev_flash/hen/xml/gall-off.xml", "/dev_rewrite/vsh/resource/explore/xmb/category_game.xml");
		cellFsUnlink("/dev_hdd0/hen/hen_gps3.on");
		cellFsUnlink("/dev_hdd0/hen/hen_gps2.on");
		cellFsUnlink("/dev_hdd0/hen/hen_gps1.on");
		cellFsUnlink("/dev_hdd0/hen/hen_gpsp.on");
		cellFsUnlink("/dev_hdd0/hen/hen_gpsn.on");
		cellFsUnlink("/dev_hdd0/hen/hen_ghb.on");
		cellFsUnlink("/dev_hdd0/hen/hen_grg.on");
		cellFsUnlink("/dev_hdd0/hen/gps3.xml");
		cellFsUnlink("/dev_hdd0/hen/gps2.xml");
		cellFsUnlink("/dev_hdd0/hen/gps1.xml");
		cellFsUnlink("/dev_hdd0/hen/gpsp.xml");
		...
		cellFsUnlink("/dev_hdd0/hen/ghb.xml");
		cellFsUnlink("/dev_hdd0/hen/grg.xml");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_gps3.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_gps2.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_gps1.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_gpsp.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_gpsn.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_ghb.off");
		filecopy("/dev_hdd0/hen/off.off", "/dev_hdd0/hen/hen_grg.off");
		filecopy("/dev_hdd0/hen/off.png", "/dev_hdd0/hen/hen_gps3.png");
		filecopy("/dev_hdd0/hen/off.png", "/dev_hdd0/hen/hen_gps2.png");
		filecopy("/dev_hdd0/hen/off.png", "/dev_hdd0/hen/hen_gps1.png");
		filecopy("/dev_hdd0/hen/off.png", "/dev_hdd0/hen/hen_gpsp.png");
		filecopy("/dev_hdd0/hen/off.png", "/dev_hdd0/hen/hen_gpsn.png");
		filecopy("/dev_hdd0/hen/off.png", "/dev_hdd0/hen/hen_ghb.png");
		filecopy("/dev_hdd0/hen/off.png", "/dev_hdd0/hen/hen_grg.png");
	}
	reload_xmb_n();
}

void toggle_hen_apphome()
{
	toggle_generic("/dev_hdd0/hen/hen_apphome", "/dev_hdd0/hen/hen_apphome.png", "apphome");
	int rt = 0;
	int ff = 0;
	CellFsStat stat;
	rt = cellFsStat("/dev_hdd0/hen/hen_apphome.off", &stat);
	if (rt != CELL_OK)
	{
		/*cellFsOpen("/dev_hdd0/hen/apphome.xml", CELL_FS_O_CREAT | CELL_FS_O_RDWR, &ff, 0, 0);
		cellFsClose(ff);
		filecopy("/dev_flash/hen/xml/apphome.xml", "/dev_hdd0/hen/apphome.xml");*/
	}
	else
	{
		cellFsUnlink("/dev_hdd0/hen/apphome.xml");
		//filecopy("/dev_flash/hen/xml/empty.xml", "/dev_hdd0/hen/apphome.xml");
	}
	reload_xmb_n();
}
...
void mount_hdd()
{
	sys_map_path("/dev_bdvd", "/dev_hdd0/packages");
	reload_xmb_g();
}

void mount_hdd_game()
{
	sys_map_path("/dev_bdvd", "/dev_hdd0/GAMES/BLES02108/PS3_GAME");
	reload_xmb_g();
}
...
void plugin_install(char* plugin_patch, char* plugin_icon)
{
	...
	rt = cellFsStat("/dev_hdd0/boot_plugins.bk", &stat);
	if (rt != CELL_OK)
	{
		if (cellFsStat("/dev_hdd0/boot_plugins.txt", &stat) == CELL_OK) filecopy("/dev_hdd0/boot_plugins.txt", "/dev_hdd0/boot_plugins.bk");
		if (cellFsOpen("/dev_hdd0/boot_plugins.txt", CELL_FS_O_RDWR | CELL_FS_O_CREAT | CELL_FS_O_APPEND, &fd, NULL, 0) == CELL_OK)
		{
			cellFsWrite(fd, buffer, size, &nrw);
			cellFsClose(fd);
			filecopy("/dev_hdd0/hen/on.png", plugin_icon);
			showMessage("plugin_install_ok", (char*)XAI_PLUGIN, (char *)TEX_SUCCESS);
		}
		else showMessage("plugin_install_bad", (char*)XAI_PLUGIN, (char *)TEX_ERROR);
	}
	else
	{
		filecopy("/dev_hdd0/hen/off.png", plugin_icon);
		filecopy("/dev_hdd0/boot_plugins.bk", "/dev_hdd0/boot_plugins.txt");
		cellFsUnlink("/dev_hdd0/boot_plugins.bk");
		showMessage("plugin_uninstall", (char*)XAI_PLUGIN, (char *)TEX_SUCCESS);
	}
	reload_xmb_n();
}
...
void akari_install()
{
	plugin_install("/dev_hdd0/plugins/Akari.sprx", "/dev_hdd0/hen/ip.png");
}
...
void uninstall_hen()
{
	int rt1, rt2, rt3 = 0;
	CellFsStat stat;
	rt1 = cellFsStat("/dev_hdd0/hen/restore/xmb/category_game.xml", &stat);
	rt2 = cellFsStat("/dev_hdd0/hen/restore/xmb/category_network.xml", &stat);
	rt3 = cellFsStat("/dev_hdd0/hen/restore/xmb/category_video.xml", &stat);

	if (rt1 == CELL_OK && rt2 == CELL_OK && rt3 == CELL_OK) {
		showMessage("wait", (char*)XAI_PLUGIN, (char *)TEX_INFO2);
		filecopy("/dev_hdd0/hen/restore/explore_category_game.rco", "/dev_rewrite/vsh/resource/explore_category_game.rco");
		filecopy("/dev_hdd0/hen/restore/explore_plugin_full.rco", "/dev_rewrite/vsh/resource/explore_plugin_full.rco");
		filecopy("/dev_hdd0/hen/restore/software_update_plugin.rco", "/dev_rewrite/vsh/resource/software_update_plugin.rco");
		filecopy("/dev_hdd0/hen/restore/sysconf_plugin.rco", "/dev_rewrite/vsh/resource/sysconf_plugin.rco");
		...
		filecopy("/dev_hdd0/hen/restore/xmb/category_game.xml", "/dev_rewrite/vsh/resource/explore/xmb/category_game.xml");
		filecopy("/dev_hdd0/hen/restore/xmb/category_network.xml", "/dev_rewrite/vsh/resource/explore/xmb/category_network.xml");
		filecopy("/dev_hdd0/hen/restore/xmb/category_video.xml", "/dev_rewrite/vsh/resource/explore/xmb/category_video.xml");
		...
		showMessage("Hen_Removed", (char*)XAI_PLUGIN, (char *)TEX_SUCCESS);
	}
